...
$("#fullscreen-button").on("click", function () {
  ...
  function updateStepCounter(currentStep, totalSteps) {
    document.getElementById(
      "step-counter"
    ).textContent = `${currentStep}/${totalSteps}`;
    drawCircularProgress(currentStep, totalSteps);
  }

  function initializeTour() {
    $("#guided-tooltip-prev").off().on("click", previousStep);
    $("#guided-tooltip-next").off().on("click", nextStep);
    $("#guided-tooltip-exit").off().on("click", endTour);

    // Add scroll event listener
    document
      .getElementById("guided-tooltip-text")
      .addEventListener("scroll", updateScrollMask);
  ...
  function updateTourUI() {
    ...
    // Update step counter and progress circle
    updateStepCounter(currentTourStep + 1, tourSteps.length);

    // Remove all highlights
    $(".highlight").removeClass("highlight");
    $("#ecosystem-highlight").css("opacity", "0");

    // Remove previous highlights and hidden elements
    $(".highlight").removeClass("highlight");
    ...
    // Handle Step 2: Highlight the Canvas and Hide Elements
    if (currentTourStep === 1) {
      // Step 2
      // Add base transition class to all elements
      const elements = {
        "#top-nav": "slide-up",
      ...
      // Reset all elements
      const elements =
        "#top-nav, .environment-info, #fullscreen-button, #contract-address-box, #snapshot-button, #logo-circle";
      $(elements)
    ...
    // Add highlight based on element
    if (stepData.element) {
    ...
    if (currentTourStep === 1) {
      ...
      $("#canvas-title, #canvas-subtitle").css({
        transition: "opacity 0s ease" // Ensure transition
      });
      setTimeout(() => {
      ...
      $("#canvas-title, #canvas-subtitle").css({
        transition: "opacity 0.6s ease" // Ensure transition
      });
      setTimeout(() => {
    ...
    // Update content and check for scrollable content
    const content = document.getElementById("guided-tooltip-text");
    ...
    // Handle Previous button state and arrow visibility
    $("#guided-tooltip-prev")
  ...
  async function fetchMarketCap() {
    const tokenAddress = "C3JX9TWLqHKmcoTDTppaJebX2U7DcUQDEHVSmJFz6K6S"; // Replace with your token address
...
if (priceChange1hr < -15) {
    ...
    conditionColor = "linear-gradient(90deg, #ff4444, #ff6666, #ff4444)";
    animationClass = "condition-animate-default";
} else {
    if (marketCap < 50000) {
        condition = "Emerging Habitat";
        conditionColor = "linear-gradient(45deg, #00C6FB, #A0D8F1, #005BEA)";
        animationClass = "condition-animate-default";
    } else if (marketCap < 100000) {
        condition = "Expanding Waters";
        conditionColor = "linear-gradient(45deg, #38EF7D, #70C1B3, #11998E)";
        // Adding a slight shake for extra emphasis
        animationClass = "condition-animate-shake";
    } else if (marketCap < 500000) {
        condition = "Vibrant Reef";
        conditionColor = "linear-gradient(45deg, #FFE259, #F3D250, #FFA751)";
        // Using a rainbow effect (animated gradient) for a vibrant look
        animationClass = "condition-animate-rainbow";
    } else if (marketCap < 1000000) {
        condition = "Dynamic Ecosystem";
        conditionColor = "linear-gradient(45deg, #FF416C, #F5C979, #FF4B2B)";
        animationClass = "condition-animate-default";
    } else if (marketCap < 2500000) {
        condition = "Thriving Biodiversity";
        conditionColor = "linear-gradient(45deg, #56AB2F, #99D98C, #A8E063)";
        animationClass = "condition-animate-default";
    } else if (marketCap < 5000000) {
        condition = "Bursting Ecosystem";
        conditionColor = "linear-gradient(45deg, #00B4DB, #57C785, #0083B0)";
        animationClass = "condition-animate-default";
    } else if (marketCap < 10000000) {
        condition = "Oceanic Metropolis";
        conditionColor = "linear-gradient(45deg, #2C3E50, #5D8AA8, #3498DB)";
        animationClass = "condition-animate-default";
    } else if (marketCap < 15000000) {
        condition = "Marine Haven";
        conditionColor = "linear-gradient(45deg, #1A2980, #355070, #26D0CE)";
        animationClass = "condition-animate-default";
    } else if (marketCap < 20000000) {
        condition = "Aquatic Utopia";
        conditionColor = "linear-gradient(45deg, #141E30, #283D3B, #243B55)";
        animationClass = "condition-animate-default";
    } else {
        condition = "Endless Ocean";
        conditionColor = "linear-gradient(45deg, #000428, #1C1C30, #004E92)";
        animationClass = "condition-animate-default";
    }
}
...
<h3 style="text-align: center; font-size: 14px; color: rgba(255, 255, 255, 0.7);">
  ...
  ${bacteriaWarning}`
      ...
      const capacityPercentage = Math.min((particles.length / 2000) * 100, 100);
  ...
  function calculateEnvironmentParams() {
    ...
    return {
      maxParticles: Math.floor(
        baseParticles + marketCapPercent * marketCapPercent * 2000
      ),
...
class Plankton {
    constructor(canvas, progress) {
        ...
        this.zIndex = 1;
        this.speciesName = "Plankton";
        this.behavior = "Clusters of microscopic drifters, each with multiple sub-particles";
        this.colorDescription = "Faint white bodies, bright aqua glow when disturbed";
...
class Coral {
    constructor(canvas, x, y, size, health = 1, allowedColors = ['cyan']) {
        ...
        this.speciesName = "Coral Formation";
        this.behavior = "Living architecture that flourishes with ecosystem health";
        this.colorDescription = "Bioluminescent polyps with ethereal glow";
...
if (mouseDistance < REPULSION_RADIUS) {
  ...
  class UndulatingSerpent {
    constructor(canvas) {
      ...
      this.colorDescription = "Glistening scales with alternating colors";
...
class Jellyfish {
    constructor(canvas) {
        ...
        this.colorDescription = "Translucent Blue";
...
gradient.addColorStop(1, `rgba(80, 130, 255, ${0.1 + pulseOpacity})`);
  ...
  class Stingray {
    constructor(canvas) {
      ...
      this.colorDescription = "Shimmering Blue";
...
function clear() {
    ...
    const maxRedIntensity = 0.1;
    const maxGreenIntensity = 0.1; // reduce this to make green less bright
...
function update(timestamp) {
  ...
  if (!isSnapshotMode) {
    ...
    particles.forEach((p) => {
      if (waveInfo.active && p instanceof Plankton) {
        ...
        if (distanceFromWaveFront < waveWidth && distanceFromWaveFront > -waveWidth) {
          // Calculate intensity based on distance from wave front
          const intensity = Math.max(0, 1 - Math.abs(distanceFromWaveFront) / waveWidth);
    ...
    // Snapshot mode - render all particles without movement
     particles.forEach((particle) => {
      if (particle) {
        // Try render first, fall back to draw if render doesn't exist
        if (particle.render) {
          particle.render();
        } else if (particle.draw) {
          particle.draw();
        }
      }
    });
